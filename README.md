...
```bash
uv tool install /path/to/codex_project_context_loader-0.1.4-py3-none-any.whl
```
...
`repository` from `global`; version 0.1.4's fixed root-file selection currently emits only
...
integer `1`, while `tool.version` is `0.1.4`. Callers must depend only on fields declared above. The
...
Version 0.1.4 does not provide AI summaries, project-type detection, nested `AGENTS.md` handling,
